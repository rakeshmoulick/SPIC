...
w_pe*t = NUM_TS*DT
"""
ndec = ndec.reshape(DATA_TS,(NC+1))
...
# plt.pcolor(K, Omega, Zc, cmap='rainbow',shading='auto',vmin=np.min(Zc),vmax=np.max(Zc))
plt.pcolor(K, Omega, Zc, cmap='rainbow',shading='auto',vmin=-7,vmax=np.max(Zc))
#print(np.min(Zc), np.max(Zc))
cbar = plt.colorbar()
...
plt.savefig(pjoin(path,'dispersion_colddensity_vd_%d.jpg'%(vd)),dpi=dpi)
